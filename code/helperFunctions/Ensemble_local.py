...
REACTION_ATTRIBUTES = ['lower_bound', 'upper_bound', 'gene_reaction_rule', 'name', 'subsystem']
MISSING_ATTRIBUTE_DEFAULT = {'lower_bound':0,'upper_bound':0,'gene_reaction_rule':'','name':'','subsystem':''}

class Ensemble_local(Object):
    """
    ...
    def _populate_features_base(self, list_of_models):
        ...
        # Collect ALL attributes from REACTION_ATTRIBUTES for each reaction in each model
        model_reaction_attrs = {}
        for model in list_of_models:
            mid = model.id
            model_reaction_attrs[mid] = {}
            for rxn in model.reactions:
                # Collect all tracked attributes, using getattr with defaults for safety
                attrs = {}
                for attr in REACTION_ATTRIBUTES:
                    try:
                        attrs[attr] = getattr(rxn, attr, MISSING_ATTRIBUTE_DEFAULT.get(attr, None))
                    except AttributeError:
                        attrs[attr] = MISSING_ATTRIBUTE_DEFAULT.get(attr, None)
                model_reaction_attrs[mid][rxn.id] = attrs
        ...
        for reaction in all_reactions:
            ...
            for model in list_of_models:
                ...
                if reaction in rxn_attrs:
                    ...
                    # Use defaults if reaction missing from model
                    rxn_vals[mid] = {attr: MISSING_ATTRIBUTE_DEFAULT.get(attr, None) 
                                     for attr in REACTION_ATTRIBUTES}
